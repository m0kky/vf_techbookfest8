...
そこで少しひねります。ユーザーが発話したレシピ名で検索ができたということは、ユーザーが発話したレシピ名が入っている変数```varUserRecipe```と、スプレッドシートから取得したレシピ名が入っている変数```varName```はイコールになっているはずですよね。これを比較すれば検索結果が取得できたかどうかがわかります。やってみましょう。
