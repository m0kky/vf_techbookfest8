...
ユーザの発話の受け取りが必要になりますので、少しフローを変更します。まず、最初のSpeakブロックの中身を修正します。
...
Interactionブロックでインテント/サンプル発話/スロットを作っていきます。Interactionブロックをクリックして、設定のSlotタブ

上のSpeakブロック


//note[インテント/サンプル発話/スロットについて]{
インテント・サンプル発話・スロットについては、私が作成したハンズオン資料にもまとめていますので、そちらもご覧ください。
