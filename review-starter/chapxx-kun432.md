...
単純に考えると、スプレッドシートから取得したデータが入っている変数```varName```か変数```varContent```が```undefined```かどうかをチェックすれば良い、つまりイコールを使って比較すれば良いと思いますよね。ところが、この```undefined```はプログラム的にいうと少し特殊で、以下のように設定してもうまく動きません。

![](images/chapxx-kun432/s092.png)

そこで少しひねります。ユーザーが発話したレシピ名で検索ができたということは、ユーザーが発話したレシピ名が入っている変数```varName```と、スプレッドシートから取得したレシピ名が入っている変数```varName```はイコールになっているはずですよね。つまりこれを比較すれば、検索結果が取得できたかどうかがわかります。やってみましょう。
...
- Googleスプレッドシートに大量のデータが存在する（行数や列数が多い、セル内の文字数が多い、等）場合、レスポンスが遅くなったり、データが取得できなくなったりする場合があります。行・列・セル内データ等の組み合わせにもよるので一概には言えませんが、多くても1000行未満に抑えることをお勧めします。
