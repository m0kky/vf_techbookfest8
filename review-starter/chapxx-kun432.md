...
そこで少しひねります。ユーザが発話したレシピ名で検索ができたということは、ユーザが発話したレシピ名が入っている変数```varName```と、スプレッドシートから取得したレシピ名が入っている変数```varName```はイコールになっているはずですよね？つまりこれを比較すれば、検索結果が取得できたかどうかがわかります。やってみましょう。
