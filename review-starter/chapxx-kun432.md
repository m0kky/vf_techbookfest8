...
検索条件となるユーザの発話を受け取る準備ができました。では、その発話をもとにスプレッドシートを検索するようにしてみましょう。Integrationブロックをクリックして、「With Settings」をクリックします。
...
ランダムの場合とは変わった画面が出てきました。```varUserRecipe```になにか入力できるようです。検索条件を変数で指定するとこのように変数の値を変えながらテストができます。試しに「カレーライス」と入力して「Run」をクリックしてみてください。
...
```undefined``` は「何も定義されていない」ということを意味しています。レシピデータにないレシピ名を検索しようとしたので当然なのですが、Alexaがこういう回答を返すのはちょっと良くないですね。少し修正しましょう。@<br>{}
...
そこで少しひねります。ユーザが発話したレシピ名で検索ができたということは、ユーザが発話したレシピ名が入っている変数```varName```と、スプレッドシートから取得したレシピ名が入っている変数```varName```はイコールになっているはずですよね？つまりこれを比較すれば、検索結果が取得できたかどうかがわかります。やってみましょう。@<br>{}

まず、上側の「Select Variable」は```varUserRecipe```を選択します。

![](images/chapxx-kun432/s098.png)

次に、下側の「Value」ですが、初期状態だと値を直接入力するだけしかできず、変数は選択できません。そこで、一番右端の```</>```アイコンをクリックします。するとメニューが表示されるので「Variable」をクリックします。
...
はい、これで、検索結果が取得できた場合は「1」のフローへ、できなかった場合は「else」のフローに進むという分岐ができました。
