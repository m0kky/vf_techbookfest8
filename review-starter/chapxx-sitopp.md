...
技術書典5と7に「温泉♨️BBA」で出展した際に、ブースに来てくださったお客さんに「スマートスピーカー何を持ってますか？」としつこく聞いてたところ、Google Home miniを持っているという人の割合が高かったので、この記事ではGoogle Homeで使えるものを作ってみたいと思います。

電子工作クラスタの人に、「音声で操作するインタフェース」は案外気軽にできる事を知ってもらいたい。そしてVUIクラスタの人にもぜひ**軽率に電子工作沼にハマっていただきたい**と思っております。
半田付けなどしなくても使える、人気の格安小型マイコン「M5StickC」を使います。
...
IoTはハードとソフトを両方使うので、ハード側とソフト側でそれぞれ単体テストをしっかりやらないと、結合してテストして動かないときの調査の手間が大きいなと思いました。MQTTの送受信のテストにenebularを使ったり、Webhooksの送信テストにターミナルからcurlコマンドを実行したり、地道なテストをしていたのですが、ページ数の都合で割愛しました。いずれそういうものもqiita等で記事にできたら良いなと思います。

#### 残念なこと

赤外線リモコンのフォーマットがメーカー毎にばらばらなので、今回のサンプルコードではDAIKINのエアコンにしか対応できなかったのが残念です。暇をみて、他のメーカーの赤外線フォーマットも調べてみようと思います。

#### よかったこと

Voiceflowを使うと、Amazon AlexaとGoogle Homeの両方に楽に対応できます。
「interaction」ブロックの「Choices」はAlexa / Googleで作りが違うので、プラットフォーム毎に用意する必要があるでしょう。とはいえ基本ロジックが使い回しでき、デプロイもボタン一発でできるのは大変便利ですね。



### VUIとハードウェアの融合について

今回初めての試みとして、電子工作ファンの間で爆発的な人気を得ている「M5」シリーズの「M5StickC」をVoiceflowから呼び出すという事もやってみました。

「M5StickC」は、別売で気温湿度などのセンサーユニットや、チョロQみたいに走らすための車輪ユニットなども販売されていて、注目のマイコンです。本稿では触れてませんが、ノンコーディングでプログラムが組めるビジュアルプログラミングツールも提供されており、お子さんのSTEM教育にも良いと思います。

AlexaやGoogle Assistantが生活のインフラの一部になるのはそう遠い未来ではないと予想されているものの、現在はまだまだ「あの筒型のスピーカーで音楽や天気を聞くもの」というイメージが強いです。
ハードウェアとの融合がもっと進んで新しいユーザー体験が生み出され、かつてアップルがiPhoneを生み出した時のような革命が起きるのを願ってやみません。そのためには一回既存のものを全部ぶっ壊した方がいいんじゃないかとすら思う今日この頃です。
